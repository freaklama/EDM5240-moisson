...
### WOW! Quel script original!
### C'est un jeu qui moissonne des données en fonction de ce que l'utilisateur entre comme information!
### Bravo pour cette idée qui marie interaction et information!
### J'ai simplifié ton script; regarde ci-dessous
...
def reponseValide(choixIndex):
    ...
    for i in page.find_all("td"):
        if i.text.strip() == choixIndex: ### Le principal changement est ici, j'ai remplacé tous les choix de valeurs que tu avais en un seul, la variable «choixIndex» que tu avais déjà définie
            print("     ",i.find_next("td").text.strip())
            continuer = str(input("Voulez-vous choisir un autre index ('oui' ou 'non')?"))
            if continuer == "oui":
                menuChoix()
            if continuer == "non":
